...
class ParseObject {
    ...
    private conc(list: ParseObject[], n: number): string {
        return this.concop(list, " ", n);
    }
    private conclist(list: ParseObject[], n: number): string {
        var s = "";
        if (list != null) {
            for (var obj of list) {
                if (obj != null) {
    ...
    private perl_prefix(str: string): boolean {
        return str == "_$" || str == "_$&" || str == "_$get_" || str == "_$set_";
    }
    private format(formatstr: string, list: ParseObject[], n: number): string {
        // $n 1行
        // @n 複数行
        var s = "";
    ...
    private format_block(x: number, list: ParseObject[], n: number, first_line: string, last_line: string): string {
        var s = "{\r\n";
        if (first_line != "") {
    ...
    private format_constructor_block(prm_n: number, block_n: number, list: ParseObject[], n: number): string {
        var prms = this.format("," + String(prm_n), this.children, n);
    ...
    private opt(ptn: string, n: number, cond: boolean): string {
    ...
    //}
    private getHeritage(heritageClauses: ParseObject): string {
        ...
        // ここの構造は不明のため全部連結して返す
        var s = "";
        for (var heritageClause of heritageClauses.list) {
            for (var expressionWithTypeArguments of heritageClause.children[0].list) {
                if (s != "") {
                    s += " ";
                }
    ...
    public toTypeScript(op: string, n: number): string {
        if (op == "nl") {
            if (this.kinde == 0) {
                if (this.list != null) {
    ...
    public toTypeScript1(op: string, n: number): string {
        switch (this.kinde) {
